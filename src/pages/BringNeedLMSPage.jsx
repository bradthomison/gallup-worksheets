...
export default function BringNeedLMSPage() {
  ...
  async function lookupByEmail(emailVal) {
    ...
    const map = {}
    strengths.forEach(s => { map[s] = { ...BRING_NEED[s] } })

    if (strengths.length > 0) {
      const { data: dbRows } = await supabase
        .from('report_content')
        .select('strength_name, content')
        .eq('report_type', 'bring_need')
          ...
          <div className="max-w-md mx-auto">
            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-8">
              <div className="mb-6">
                ...
                <h1 className="text-2xl font-bold text-gray-900">Bring - Need</h1>
                <p className="text-sm text-gray-500 mt-1">
                  Enter the email address your coach has on file to view your Bring - Need report.
                </p>
              </div>
            ...
            <div className="flex items-start justify-between mb-6 flex-wrap gap-3">
              ...
              <div className="flex items-center gap-2 print:hidden">
                <button
                  onClick={async () => {
                    setPdfLoading(true)
                    await downloadBringNeedPDF({ ...person, top5: strengths })
                    setPdfLoading(false)
                  }}
                  disabled={pdfLoading}
                ...
                <button
                  onClick={() => {
                    const html = buildBringNeedPrintHTML(person.name, strengths)
              ...
              <table className="w-full border-collapse text-sm">
                ...
                <thead>
                  <tr>
                    ...
                    <th className="border border-gray-200 bg-gray-50 p-3 text-left text-xs font-semibold text-gray-700">
                      I Bring
                ...
                <tbody>
                  {strengths.map(s => {
                    ...
                    return (
                      <tr key={s}>
                        <th
                          className="border border-gray-200 p-3 text-left text-xs font-bold align-top"
                          style={{ background: c.headerBg, color: c.headerText, printColorAdjust: 'exact', WebkitPrintColorAdjust: 'exact' }}
                        >
                          {s}
                        </th>
                        <td className="border border-gray-200 p-3 text-xs text-gray-700 align-top leading-relaxed" style={{ whiteSpace: 'pre-line' }}>
                          {insights?.[s]?.bring ?? ''}
                        </td>
                        <td className="border border-gray-200 p-3 text-xs text-gray-700 align-top leading-relaxed" style={{ whiteSpace: 'pre-line' }}>
                          {insights?.[s]?.need ?? ''}
